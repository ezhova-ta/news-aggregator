    ...
    private static final class ShowRssChannelListTask extends AsyncTask<Void, Void, AsyncTaskResult<List<RssChannel>>> {
        ...
        protected void onPostExecute(final AsyncTaskResult<List<RssChannel>> result) {
            ...
            if(result.getException() != null) {
                presenter.get().deletingRssChannelListView.showRssChannelsLoadingErrorMessage();
            } else {
                if(result.getResult().isEmpty()) {
                    presenter.get().deletingRssChannelListView.hideConfirmDeletingRssChannelsButton();
                    presenter.get().deletingRssChannelListView.startActivityToDisplayRssChannelList();
                }
